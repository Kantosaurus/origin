...
async fn spawned_workers_run_concurrently_not_serially() {
    ...
    // ever seen, then waits on a shared barrier before decrementing. The barrier
    // releases only once all THREE workers have arrived, so every worker is
    // provably in-flight simultaneously — overlap is forced by synchronization,
    // not by guessing a sleep window that outlasts scheduling jitter.
    let current = Arc::new(AtomicUsize::new(0));
    let max_seen = Arc::new(AtomicUsize::new(0));
    let barrier = Arc::new(Barrier::new(WORKERS));
    let (cur2, max2, bar2) = (Arc::clone(&current), Arc::clone(&max_seen), Arc::clone(&barrier));
    let worker: WorkerFn = Arc::new(move |ctx: WorkerContext| {
        let (cur, mx, bar) = (Arc::clone(&cur2), Arc::clone(&max2), Arc::clone(&bar2));
        Box::pin(async move {
            let now = cur.fetch_add(1, Ordering::SeqCst) + 1;
            mx.fetch_max(now, Ordering::SeqCst);
            // All N must be here at once for this to return — proves overlap.
            bar.wait().await;
